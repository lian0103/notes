...
const outputPath = path.resolve(__dirname, '../docs/.vuepress/dist/sitemap.xml');
...
jsonData.forEach((data) => {
    data.notes.forEach((item) => {
        ...
        url.ele('loc', {}, `${domain}/${data.folder}/${item.file.replace('.md', '.html')}`);
        url.ele('changefreq', {}, data.changefreq || '');
        url.ele('priority', {}, data.priority || '');
    });
});
